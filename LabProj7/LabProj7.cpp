...
{
    if (llist == nullptr || startIndex > endIndex) {
        return;
    }
    ...
    while (deleter != nullptr && counter < startIndex) {
        prev = deleter;
    ...
    while (deleter != nullptr && counter <= endIndex) {
        nextNode = deleter->next;
        delete deleter;
        deleter = nextNode;
        counter++;
    }
...
void MoveSmallestToHead(Node*& llist)
{
    ...
    Node* smallestNode = llist;
    Node* prev = nullptr;
    Node* curr = llist;

    while (curr->next)
    {
        if (curr->next->data < smallestNode->data)
        {
            prevSmallest = curr;
            smallestNode = curr->next;
        }
        curr = curr->next;
    }

    if (smallestNode == llist)
    {
        return;
    }
    ...
    if (prevSmallest)
    {
        prevSmallest->next = smallestNode->next;
    }


    smallestNode->next = llist;
    llist = smallestNode;
